...
function login($inputID, $inputPW) {
    ...
    // 입력받은 정보로부터 SQL Query 작성
        // PW는 HASH값 사용 - bcrypt 해시 알고리즘의 경우 같은 값이더라도, 매번 다른 값이 나오므로 SQL에서 사용 불가
        // 따라서 SHA 256 알고리즘으로 구현함.
    $sql = "SELECT `id`, `pw` FROM week3 WHERE `id` = '".$inputID."' AND `pw` = '".hash('sha256', $inputPW)."';";
